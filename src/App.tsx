...
export default function App() {
  ...
  useEffect(() => {
    (window.organizer?.getConfig() ?? Promise.resolve(browserFallback())).then((data) => { setConfig(data); setLoading(false); });
  }, []);
  useEffect(() => {
    if (!window.organizer) return;
    config.apps.forEach((entry) => { if (!icons[entry.id]) window.organizer!.getIcon(entry.id).then((icon) => icon && setIcons((old) => ({ ...old, [entry.id]: icon }))); });
  }, [config.apps]);
  ...
  const removeApp = async (entry: AppEntry) => { if (confirm('从整理工具中移除“' + entry.name + '”？本机程序不会被删除。')) await persist({ ...config, apps: config.apps.filter((item) => item.id !== entry.id) }); };
  const removeCategory = async (category: Category) => {
    ...
    await persist({ ...config, categories: config.categories.filter((item) => item.id !== category.id), apps: config.apps.filter((item) => item.categoryId !== category.id) }); setSelected('all');
  };
  const moveCategory = async (index: number, offset: number) => { const list = [...categories]; const other = index + offset; if (other < 0 || other >= list.length) return; [list[index], list[other]] = [list[other], list[index]]; await persist({ ...config, categories: list }); };
  const reorderApp = async (targetId: string) => {
    ...
    await persist({ ...config, apps: config.apps.map((item) => orders.has(item.id) ? { ...item, order: orders.get(item.id)! } : item) });
  ...
  return <div className="app-shell">
    <header className="titlebar">
      ...
      <div className="title-actions">
        <button className="icon-button" title="导入配置" onClick={async () => { try { const value = await window.organizer?.importConfig(); if (value) { setConfig(value); flash('配置已导入'); } } catch (e) { flash(e instanceof Error ? e.message : '导入失败'); } }}><Import size={18} /></button>
        <button className="icon-button" title="导出配置" onClick={async () => { if (await window.organizer?.exportConfig()) flash('配置已导出'); }}><Upload size={18} /></button>
    ...
    <div className="workspace">
      ...
      <main className="content">
        ...
        {loading ? <div className="empty-state"><span className="loader" />正在读取本地配置</div> : shownApps.length ? <div className="app-grid">{shownApps.map((entry, index) => <AppCard key={entry.id} entry={entry} icon={icons[entry.id]} color={palette[index % palette.length]} dragging={draggedAppId === entry.id} onDragStart={() => setDraggedAppId(entry.id)} onDrop={() => reorderApp(entry.id)} onDragEnd={() => setDraggedAppId(null)} onLaunch={() => launch(entry)} onEdit={() => { setEditingApp(entry); setDialog('app'); }} onDelete={() => removeApp(entry)} />)}</div> : <div className="empty-state"><AppWindow size={42} /><h2>{query ? '没有匹配结果' : '这个分类还是空的'}</h2><p>{query ? '换一个关键词试试。' : '添加程序、文件夹、文件或网页入口。'}</p>{!query && <button className="primary-button" onClick={() => setDialog('app')}><Plus size={18} />添加第一个项目</button>}</div>}
      </main>
    </div>
    {dialog === 'app' && <AppDialog categories={categories} entry={editingApp} onClose={() => setDialog(null)} onSave={async (entry) => { const exists = config.apps.some((x) => x.id === entry.id); const savedEntry = exists ? entry : { ...entry, order: config.apps.filter((item) => item.categoryId === entry.categoryId).length }; await persist({ ...config, apps: exists ? config.apps.map((x) => x.id === entry.id ? savedEntry : x) : [...config.apps, savedEntry] }); setDialog(null); flash(exists ? '项目已更新' : '项目已添加'); }} />}
    {dialog === 'category' && <CategoryDialog category={editingCategory} onClose={() => setDialog(null)} onDelete={editingCategory ? () => { removeCategory(editingCategory); setDialog(null); } : undefined} onSave={async (category) => { const exists = config.categories.some((x) => x.id === category.id); await persist({ ...config, categories: exists ? config.categories.map((x) => x.id === category.id ? category : x) : [...config.categories, category] }); setDialog(null); }} />}
    {dialog === 'settings' && <SettingsDialog onClose={() => setDialog(null)} onReveal={() => window.organizer?.revealConfig()} onReset={async () => { if (confirm('清空当前列表并恢复默认分类？')) { await persist({ ...demoConfig, apps: [] }); setDialog(null); } }} />}
...
function AppDialog({ categories, entry, onClose, onSave }: { categories: Category[]; entry: AppEntry | null; onClose(): void; onSave(entry: AppEntry): void }) {
  const [form, setForm] = useState<AppEntry>(entry ?? { id: uid(), categoryId: categories[0]?.id ?? '', name: '', description: '', target: '', targetType: 'executable', args: [], workingDirectory: '', iconPath: '', order: 0, launchCount: 0, lastLaunchedAt: null });
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="modal"><header><div><h2>{entry ? '编辑项目' : '添加项目'}</h2><p>只保存入口，不会复制或移动目标文件。</p></div><button className="icon-button" title="关闭" onClick={onClose}><X size={19} /></button></header><div className="form-grid">
    ...
    <label className="full"><span>自定义图标</span><div className="input-action"><input value={form.iconPath} onChange={(e) => update({ iconPath: e.target.value })} placeholder="可选，默认读取目标图标" /><button title="选择图标" onClick={async () => { const value = await window.organizer?.pickIcon(); if (value) update({ iconPath: value }); }}><FolderOpen size={17} /></button></div></label>
