  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="modal quick-add-modal">
      ...
      <div className="quick-add-body">
        ...
        {resolved && <div className={'move-preview ' + (plan?.eligible ? 'eligible' : 'blocked')}><strong>{plan ? (plan.eligible ? '可以统一收纳' : '仅添加入口') : '正在检查是否可移动'}</strong><p>{plan?.reason ?? '正在检查安装位置、目录结构和冲突。'}</p>{plan?.eligible && <><small>原位置：{plan.sourceRoot}</small><small>新位置：{plan.destination}</small></>}{Boolean(plan?.runningProcesses.length) && <p>请先正常关闭：{plan!.runningProcesses.join('、')}</p>}</div>}
      </div>
