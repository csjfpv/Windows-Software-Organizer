...
让 Codex、DeepSeek、Harness 等 Agent 帮忙安装或整理软件时，点顶栏“交给 AI 管理”，复制提示词后直接发给 Agent。提示词每次都会带入当前分类和这台电脑的启动台配置路径，明确授权 Agent 新建、重命名、排序或删除启动台分类和入口；当现有分类不合适时，Agent 可以按用途新建清晰分类。该授权只适用于启动台 JSON 配置，不允许它任意扫描电脑、移动原文件或收纳固件、设备数据、日志、密钥和个人资料。
